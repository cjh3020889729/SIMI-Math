...
void power_iterator_direct_solve()
{
    std::cout << "1.The Direct Power Iterator:\n";
    ...
    enginevalue_count = SIMI::MATRIX::power_iterator_find_eigenvalue_direct(a, init_x, output, iter_max);
    if(!SIMI::SOLVES::power_iterator_result_solve(
...
void power_iterator_aitken_solve()
{
    std::cout << "2.The Aitken Power Iterator:\n";
    SIMI::Tensor<SIMI::Float64> a(3, 3);
    a.iloc(0, 0) = 8;
    a.iloc(0, 1) = -8;
    a.iloc(0, 2) = -4;
    ...
    SIMI::u32 enginevalue_count = 0;
    SIMI::u32 iter_max = 12;
    enginevalue_count = SIMI::MATRIX::power_iterator_find_eigenvalue_aitken(a, init_x, output, iter_max);
    if(!SIMI::SOLVES::power_iterator_result_solve(
        output, enginevalue_count, engine_values, engine_vectors)) {
        std::cout << "The enginevalue count is error(>=1 && <=2).\n";
    }
...
int main()
{
    ...
    power_iterator_direct_solve();
    power_iterator_aitken_solve();
